...
class RawAssembly(AbstractAssemblyInstruction):
    ...
    def consume(cls, parser: "Parser", scope: ParsingScope) -> "RawAssembly":
        return cls(parser.consume([IdentifierToken, IntegerToken]), parser.try_consume(IntegerToken))

    def __init__(self, opcode_or_name: IdentifierToken | IntegerToken, arg: IntegerToken = None):
        self.opcode_or_name = opcode_or_name
        self.arg = arg

    def __eq__(self, other):
        return type(self) == type(other) and self.opcode_or_name == other.opcode_or_name and self.arg == other.arg
    ...
    def emit_bytecodes(
        ...
        return [
            Instruction.create_with_token(self.opcode_or_name, function, -1, opcode, arg=0 if self.arg is None else int(self.arg.text)),
        ]
